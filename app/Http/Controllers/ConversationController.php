    ...
    public function store(Request $request){
        if (Auth::check()){
            ...
            for ($i =0 ; $i < count($conversations) ; $i++){
                if( (($conversations[$i]->user_id == $request->user_id) || ($conversations[$i]->second_user_id == $request->user_id))&& (($conversations[$i]->auth_user == $request->auth_user) && ($conversations[$i]->auth_second_user == $request->auth_second_user)) ){
                    $bool = true;
