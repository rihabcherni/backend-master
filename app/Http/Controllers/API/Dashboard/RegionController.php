...
class RegionController extends Controller{
    ...
    public function GoogleMapId($id){
        ...
        foreach($etabs as $etb){
                ...
                $responsable_etablissement_id=$etb->responsable_etablissement_id;
                $responsable_etablissement=  Responsable_etablissement::where('id',$responsable_etablissement_id)->first();
                ...
                foreach($blocetabs as $blocetab){
                    ...
                    foreach($etageetabs as $etageetab){
                        $blocpoubelles= $etageetab->bloc_poubelles;
                        foreach($blocpoubelles as $blocpoubelle){
                            // $blocpoubelle->withCount('poubelles')->get();
                            $poubelles= $blocpoubelle->poubelles;
                            $nombrePoubelleEtageTotal+=$blocpoubelle->poubelles->count();

                            foreach ($poubelles as $poubelle){
                                $capacite_poubelle= $poubelle->capacite_poubelle;
                                $sum= ($poubelle->Etat* $capacite_poubelle)/100;
                ...
                array_push($etage,[
                    ...
                    "nbr_etablissement"=>$nombrePoubelleEtageTotal,
                ]);
         array_push($eta,$etb);
        }
     if($nbrPlastique!==0){
